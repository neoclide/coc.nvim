...
class Complete {
    ...
    filterResults(results, input, cword) {
        let arr = [];
        let { fuzzy } = this;
        ...
        for (let i = 0, l = results.length; i < l; i++) {
            ...
            for (let item of items) {
                ...
                if (firstMatch && cFirst && cFirst !== first)
                    continue;
                // filter unnecessary no kind results
                if (!item.kind && (input.length == 0 || word == cword || word == input))
                    continue;
                ...
                if (fuzzy)
                    item.score = fuzzaldrin_1.score(item.word, input) + (item.kind ? 0.01 : 0);
                arr.push(item);
            }
        }
        if (fuzzy) {
