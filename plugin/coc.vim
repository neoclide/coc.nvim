...
function! s:Enable(initialize)
  ...
  augroup coc_nvim
    ...
    autocmd CursorMoved         * call s:Autocmd('CursorMoved', +expand('<abuf>'), [line('.'), col('.')], synIDattr(synID(line('.'), col('.') - 1, 1), 'name'))
    autocmd CursorMovedI        * call s:Autocmd('CursorMovedI', +expand('<abuf>'), [line('.'), col('.')], synIDattr(synID(line('.'), col('.') - 1, 1), 'name'))
    autocmd CursorHold          * call s:Autocmd('CursorHold', +expand('<abuf>'), [line('.'), col('.')], coc#util#variables(bufnr('%')))
