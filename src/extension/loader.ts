...
export function createExtension(id: string, filename: string, isEmpty: boolean, options?: ExtensionLoadOptions): ExtensionExport {
  if (isEmpty || (!options?.sourceCode && !fs.existsSync(filename))) {
    return emptyExtension()
  }
  // The sourceCode bundle generated by coc-test is always CommonJS; module
  // format detection only applies to on-disk entries.
  if (!options?.sourceCode && resolveModuleFormat(filename) === 'module') throw requireESMError(filename)
  disposeExtension(id)
...
export async function createExtensionAsync(id: string, filename: string, isEmpty: boolean, options?: ExtensionLoadOptions): Promise<ExtensionExport> {
  if (isEmpty || (!options?.sourceCode && !fs.existsSync(filename))) return emptyExtension()
  disposeExtension(id)
  const logger = getLogger(!global.__isMain && !global.__TEST__, id)
  let api: unknown
