...
export interface ExtensionLoadOptions {
  /**
   * Complete bundle source of the extension entry, as generated by coc-test.
   * When provided the entry is compiled from this source directly, so the
   * JavaScript is never looked up through the package.json `main` field.
   */
  sourceCode?: string
  /**
   * Format of sourceCode. sourceCode remains CommonJS by default for
   * compatibility with existing callers.
   */
  sourceFormat?: 'commonjs' | 'module'
  /**
   * Filename used as the ESM entry identifier and for import.meta.url.
   */
  sourceFilename?: string
  /**
   * Extension root used for module resolution and the API extensionRoot.
   * Defaults to the directory of the entry filename.
   */
...
export class ExtensionLoader {
   ...
   */
  public loadJavaScript(filename: string, parent?: ExtensionCommonJSModule, source?: string): unknown {
    const cacheKey = this.normalizeFilename(filename)
    const cached = this.runtime.cjsModules.get(cacheKey)
    if (cached) return cached.exports
    const module = this.createModule(cacheKey, parent)
    ...
    try {
      this.compileCommonJS(module, code)
      module.loaded = true
    } catch (e) {
      this.failModule(module)
      throw e
 ...
 */
export function createExtension(id: string, filename: string, isEmpty: boolean, options?: ExtensionLoadOptions): ExtensionExport {
  if (isEmpty || (options?.sourceCode == null && !fs.existsSync(filename))) {
    return emptyExtension()
  }
  if (options?.sourceFormat === 'module' || (!options?.sourceCode && resolveModuleFormat(filename) === 'module')) {
    throw requireESMError(options?.sourceFilename ?? filename)
  }
  disposeExtension(id)
  ...
  const loader = getLoader(runtime)
  try {
    return finalizeExports(id, loader.loadJavaScript(runtime.entry, undefined, options?.sourceCode))
  } catch (e) {
    disposeExtension(id)
    throw e
  }
...
export async function createExtensionAsync(id: string, filename: string, isEmpty: boolean, options?: ExtensionLoadOptions, subscriptions?: Disposable[]): Promise<ExtensionExport> {
  ...
  const logger = getLogger(!global.__isMain && !global.__TEST__, id)
  let api: unknown
  // When the module is required for test
  ...
  try {
    if (options?.sourceFormat === 'module' || (sourceCode == null && resolveModuleFormat(filename) === 'module')) {
      ...
      defaultImport = loader.loadJavaScript(filename, undefined, sourceCode)
    }
