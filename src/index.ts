...
export default class CompletePlugin {
  ...
  public async cocStart(args: [CompleteOption]): Promise<void> {
    ...
    let sources = await completes.getSources(nvim, opt)
  ...
  public async cocTextChangedI(args:any): Promise<void> {
    ...
    if (character && !increment.isActivted) {
      ...
      if (isWord(character)) {
        ...
        let source = languages.getCompleteSource(languageId)
        if (source && source.shouldTriggerCompletion(character, languageId)) {
          shouldTrigger = true
        }
      }
      if (shouldTrigger) {
