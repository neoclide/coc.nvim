  ...
  private handleApplyWorkspaceEdit(
    ...
    if (workspaceEdit.documentChanges) {
      for (const change of workspaceEdit.documentChanges) {
        let c = change as TextDocumentEdit
        if (c.textDocument.version && c.textDocument.version >= 0) {
          let textDocument = openTextDocuments.get(c.textDocument.uri)
          if (textDocument && textDocument.version !== c.textDocument.version) {
