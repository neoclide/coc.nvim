...
describe('ExtensionManager', () => {
  ...
  describe('activateExtensions()', () => {
    ...
    it('should not throw when autoActivated throws', async (t) => {
      ...
      let spy = t.mock.method(manager, 'checkAutoActivate' as any, () => {
        throw new Error('test error')
      })
    ...
    it('should automatically activated', async (t) => {
      ...
      let manager = create(tmpfolder)
      let spy = t.mock.method(workspace, 'checkPatterns', () => {
  ...
  describe('activate()', () => {
    ...
    it('should log failed dependency name', async (t) => {
      ...
      let result = await manager.activate('coc-ext-main')
      assert.strictEqual(result, false)
      assert.ok(spy.mock.calls.some(call => call.arguments[0].includes('coc-ext-dep') && call.arguments[1] != null))
    })
  ...
  describe('registerExtension()', () => {
    ...
    it('should not throw on register error', async (t) => {
      let manager = create()
      let spy = t.mock.method(manager, 'registerExtension', () => {
  ...
  describe('watchExtension()', () => {
    it('should throw when watchman not found', async (t) => {
      ...
      let res = await manager.loadExtension(extFolder)
      assert.strictEqual(res, true)
      let spy = t.mock.method(workspace.fileSystemWatchers, 'getWatchmanPath', () => {
      ...
      await assert.rejects(fn(), Error)
      spy.mock.restore()
      await assert.rejects(helper.doAction('watchExtension', 'not_exists_extension'), /not found/)
    })
    ...
    it('should reload extension on file change', async (t) => {
      ...
      let called = false
      let fn = t.mock.fn()
      let r = t.mock.method(workspace, 'getWatchmanPath', () => {
        return 'watchman'
      })
      let s = t.mock.method(manager, 'reloadExtension', () => {
        fn()
        return Promise.resolve()
      })
      let spy = t.mock.method(workspace.fileSystemWatchers, 'createClient', () => {
        return {
          dispose: () => {},
    ...
    it('should watch single file extension', async (t) => {
      ...
      await assert.rejects(fn(), Error)
      let called = false
      let spy = t.mock.method(manager, 'loadExtensionFile', () => {
        called = true
        return Promise.resolve('')
      })
      await helper.waitValue(() => {
