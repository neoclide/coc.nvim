...
describe('Registry', () => {
  ...
  it('should parse extension name', () => {
    ...
    // assert.strictEqual(parseSource(`\n\n${filepath.slice(0, -3)}:1:1`), undefined)
    assert.strictEqual(parseSource(`\n\n/a/b:1:1`), 'coc.nvim')
...
describe('errors', () => {
  it('should return errors', () => {
    assert.ok((errors.directoryNotExists('dir').message).includes('dir'))
...
describe('utility', () => {
  ...
  it('should check isRunning', (t) => {
    ...
    let spy = t.mock.method(process, 'kill', () => {
  ...
  it('should run command with timeout', async () => {
    await assert.rejects(runCommand('sleep 2', { cwd: __dirname }, 0.01), errors.CancellationError)
  })
  ...
  it('should run command with Cancellation token', async () => {
    ...
    await assert.rejects(runCommand('sleep 2', { cwd: __dirname, encoding: 'unknown' }, token), errors.CancellationError)
  })
  ...
  it('should throw on command error', async () => {
    await assert.rejects(runCommand('command_not_exists', { cwd: __dirname }), Error)
  })
...
describe('terminate', () => {
  ...
  it('should terminate on other platform', (t) => {
    ...
    assert.strictEqual(res, true)
    let spy: any = t.mock.method(cp, 'execFileSync', () => {
      return undefined
    })
    ...
    assert.strictEqual(res, true)
    spy.mock.restore()
    spy = t.mock.method(cp, 'spawnSync', () => {
      throw new Error('bad')
    })
    child = spawn('ls', [], { detached: true })
...
describe('diff', () => {
  ...
  function mockElapsedTime(mock: MockTracker): NodeMock<any> {
    let now = 0
    return mock.method(Date, 'now', () => {
      now += 20
      return now
    })
  }
  ...
  describe('async', () => {
    it('should do filter', async (t) => {
      ...
      let spy = mockElapsedTime(t.mock)
      try {
