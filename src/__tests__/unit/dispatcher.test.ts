...
describe('mcp dispatcher', () => {
  ...
  function messages(): any[] {
    return (socket.write as unknown as NodeMock<any>).mock.calls.map(call => JSON.parse((call.arguments[0] as Buffer).toString('utf8')))
  }
  ...
  it('returns an internal error when a resource provider throws unexpectedly', async (t) => {
    initializeSession()
    t.mock.method(server.resources, 'read', () => Promise.reject(new Error('resource failed')))
    await handleMessage(server, session, {
      jsonrpc: '2.0',
      id: 1,
  ...
  it('normalizes compliant and legacy tool results', () => {
    ...
    normalized = normalizeResult({ content: [{ type: 'image' }] }, '2024-11-05')
    assert.deepStrictEqual(normalized.content, [{ type: 'text', text: JSON.stringify({ content: [{ type: 'image' }] }) }])
  })
