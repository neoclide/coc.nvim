...
describe('mcp lsp helpers', () => {
  ...
  it('queries provider-backed hover, signature, symbols and locations directly', async (t) => {
    ...
    let provider = t.mock.method(languages, 'hasProvider', () => (true))
    let hover = t.mock.method(languages, 'getHover', () => Promise.resolve([{ contents: 'hover' } as Hover]))
    let signature = t.mock.method(languages, 'getSignatureHelp', () => Promise.resolve({ signatures: [{ label: 'fn()' }] }))
    let symbols = t.mock.method(languages, 'getDocumentSymbol', () => Promise.resolve([]))
    try {
      ...
      let query: any = {
        ...
        fetch: t.mock.fn(() => Promise.resolve([{ uri: 'file:///target', range: Range.create(0, 0, 0, 1) }])),
        serviceMethod: 'textDocument/definition',
        buildParams: t.mock.fn()
      }
      let result = await getLocationResult(doc, Position.create(0, 2), undefined, query, CancellationToken.None)
      assert.strictEqual('locations' in result && result.locations[0].uri, 'file:///target')
    } finally {
      provider.mock.restore()
      hover.mock.restore()
      signature.mock.restore()
  ...
  it('turns provider exceptions and missing providers into query errors', async (t) => {
    ...
    let provider = t.mock.method(languages, 'hasProvider', () => (false))
    try {
    ...
    provider = t.mock.method(languages, 'hasProvider', () => (true))
    let hover = t.mock.method(languages, 'getHover')
    hover.mock.mockImplementationOnce(() => Promise.reject('hover failed'), 0)
    hover.mock.mockImplementationOnce(() => Promise.reject(new Error('hover error')), 1)
    let signature = t.mock.method(languages, 'getSignatureHelp')
    signature.mock.mockImplementationOnce(() => Promise.reject(new Error('signature failed')), 0)
    signature.mock.mockImplementationOnce(() => Promise.reject('signature string'), 1)
    let symbols = t.mock.method(languages, 'getDocumentSymbol')
    symbols.mock.mockImplementationOnce(() => Promise.reject('symbols failed'), 0)
    symbols.mock.mockImplementationOnce(() => Promise.reject(new Error('symbols error')), 1)
    try {
      ...
      let query: any = {
        ...
        fetch: t.mock.fn(() => Promise.reject('location failed')),
        serviceMethod: 'textDocument/definition',
        buildParams: t.mock.fn()
      }
      ...
      query.fetch = t.mock.fn(() => Promise.reject(new Error('location error')))
  ...
  it('hasProvider handles provider manager errors', (t) => {
    let spy = t.mock.method(languages, 'hasProvider', () => { throw new Error('failed') })
    try {
  ...
  it('summarizes service capabilities with absent optional data', () => {
    ...
    let summary = serviceCapabilitySummary(stat)
    assert.strictEqual(summary.capabilities, null)
    assert.strictEqual(summary.serverInfo, null)
    summary = serviceCapabilitySummary(stat, { client: { initializeResult: { capabilities: { hoverProvider: true }, serverInfo: { name: 'server' } } } })
    assert.deepStrictEqual(summary.capabilities, { hoverProvider: true })
    assert.deepStrictEqual(summary.serverInfo, { name: 'server' })
    summary = serviceCapabilitySummary(stat, { client: {} })
    assert.strictEqual(summary.capabilities, null)
    assert.strictEqual(summary.serverInfo, null)
  })
