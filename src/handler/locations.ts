...
export default class LocationsHandler {
  ...
  public async references(includeDeclaration = true): Promise<Location[]> {
    const { doc, position } = await this.handler.getCurrentState()
    this.handler.checkProvier('reference', doc.textDocument)
    await doc.synchronize()
    const tokenSource = new CancellationTokenSource()
    return languages.getReferences(doc.textDocument, { includeDeclaration }, position, tokenSource.token)
  }
